...
class Plugin(BasePlugin):
    ...
    def __init__(self, name: str, plugin_config: Dict[str, Any]):
        ...
        self.logger.debug("Plugin __init__ called with name: %s and config: %s", name, plugin_config)
    ...
    def initialize(self, config: Dict[str, Any]):
        ...
        if not self._dsn:
            self.logger.error("Initialization failed: 'dsn' is a required argument for the postgres plugin!")
        ...
        self.logger.info(
            "PostgreSQL plugin initialized with dsn='%s', pg_schema='%s', duckdb_alias='%s', read_only=%s, secret='%s'",
            self._dsn,
            self._pg_schema,
            self._duckdb_alias,
            self._read_only,
